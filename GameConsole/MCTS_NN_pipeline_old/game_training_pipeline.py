...
class GameDataset(Dataset):
    ...
    def _ingest_match_result(self, match_result, source_name):
        ...
        match_id = match_result.get('MatchId', source_name)
        for i, move in enumerate(match_result.get('Moves') or []):
            ...
            self._append_sample(move, source_name, i, default_value_target=value_target, match_id=match_id)

    def _append_sample(self, sample, source_name, sample_index, default_value_target=0.0, match_id=None):
        try:
            ...
            policy_target = sample.get('PolicyTarget', sample.get('policy_target'))
            if policy_target is not None:
                policy_target = np.array(policy_target, dtype=np.float32)
                if self.validate_shapes:
                    assert policy_target.shape == (ActionSpace.TOTAL_PRIMARY_ACTIONS,), f"Sample {sample_index} in {source_name}: policy_target shape {policy_target.shape} != ({ActionSpace.TOTAL_PRIMARY_ACTIONS},)"
            ...
            self.data.append({
                ...
                'match_id': match_id or sample.get('MatchId', sample.get('match_id', source_name)),
            })
        except Exception as e:
    ...
    def __getitem__(self, idx):
        ...
        if sample['policy_target'] is None:
            ...
            if 0 <= sample['action_taken'] < ActionSpace.TOTAL_PRIMARY_ACTIONS:
                policy_target[sample['action_taken']] = 1.0
        else:
...
def _policy_loss(policy_logits: torch.Tensor, policy_target: torch.Tensor) -> torch.Tensor:
    policy_target = policy_target / policy_target.sum(dim=1, keepdim=True).clamp_min(1e-8)
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Policy loss: soft cross-entropy on visit-count targets if available,
        # otherwise the target is one-hot at the selected action.
        policy_loss = _policy_loss(policy_logits, policy_target)
    ...
    return {
        ...
        'total_loss': (total_policy_loss + value_weight * total_value_loss) / max(total_samples, 1),
    }
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader:
            ...
            if val_loss['total_loss'] < best_val_loss - 1e-8:
            ...
            if patience is not None and no_improve_epochs >= patience:
                logger.info(f"Early stopping: no improvement for {no_improve_epochs} epochs (patience={patience})")
                break
